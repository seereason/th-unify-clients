{-# LANGUAGE LambdaCase, TemplateHaskell, CPP #-}
...
internalDeriveSafeCopy deriveType versionId kindName egValueType =
  egValueType >>= go . decomposeType
  where
    go :: [Type] -> Q [Dec]
    go [ForallT _tvs _cx typ] = go (decomposeType typ)
    go typ@(ConT tyName : _) = do
      info <- reify tyName
      internalDeriveSafeCopy' deriveType versionId kindName (composeType typ) info
    go typ = error ("deriveSafeCopy - no support for type: " ++ pprint (composeType typ))

internalDeriveSafeCopy' :: DeriveType -> Version a -> Name -> Type -> Info -> Q [Dec]
internalDeriveSafeCopy' deriveType versionId kindName typ info =
  case info of
    TyConI dec ->
        let (context, tyvars, cons) = decInfo dec in
        if length cons > 255
        then (fail $ "Can't derive SafeCopy instance for: " ++ pprint typ ++
                     ". The datatype must have less than 256 constructors.")
        else worker context tyvars (zip [0..] cons)
    FamilyI _ insts -> do
      decs <- forM insts $ \inst ->
        let (context, name, typarams, cons) = instInfo inst in
            if typ == composeType (ConT name : typarams)
  ...
  where
    ...
    worker' tyBase context tyvars cons =
      let typ' = composeType (typ : fmap (VarT . toName) tyvars) in
      tyBase >>= \tyBase' ->
      ((fmap VarT . Set.toList) <$> nonPhantom typ') >>= \tyvars' ->
      let ty = foldl appT tyBase [ varT $ tyVarName var | var <- tyvars ]
          safeCopyClass args = toClass ''SafeCopy args
    ...
    decInfo (DataD context _name tyvars _kind cons _derivs) = (context, tyvars, cons)
    decInfo (NewtypeD context _name tyvars _kind con _derivs) = (context, tyvars, [con])
    instInfo (DataInstD context name typarams _kind cons _derivs) = (context, name, typarams, cons)
    instInfo (NewtypeInstD context name typarams _kind con _derivs) = (context, name, typarams, [con])
    instInfo inst = error $ "Can't derive SafeCopy instance for: " ++ show (typ, inst)
#else
    decInfo (DataD context _name tyvars cons _derivs) = (context, tyvars, cons)
...
#else
    toClass name args = classP name args
#endif
