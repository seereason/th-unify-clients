...
module Data.THUnify.SafeCopyInstance where
...
import Data.THUnify.Monad ({-constraints,-} findTypeVars, M, pprint1, runM, toName)
...
--safeCopyInstance 2 'extension ''T
--
...
--   'safeCopyInstanceSimple', but never more (as we don't call
--   'getSafePut'/'getSafeGet' for types that aren't needed).
--
--   Note that you may use 'safeCopyInstanceSimple' with one
...
--   'safeCopyInstance' in that we always use 'safePut' and
...
--   @
--data T a b = C a a | D b Int
--safeCopyInstanceSimple 1 'base ''T
--   @
--
--   we generate
--
...
--   'safeCopyInstanceSimple'), but we also always write a 'Word8'
...
--   'safeCopyInstance' or 'safeCopyInstanceSimple', but may use more
...
internalSafeCopyInstanceIndexedType' deriveType versionId kindName typ tyIndex subst tvs' info = do
  ...
  case info of
    ...
    _ -> fail $ "Can't derive SafeCopy instance for: " ++ show (typ, info)
  where
    -- typeNameStr = pprint1 (foldl1 AppT (typ:tyIndex))
    worker' :: TypeQ -> (Set Name, Set Name) -> Cxt -> [TyVarBndr] -> [(Integer, Con)] -> M [Dec] ()
    worker' tyBase (pvs, uvs) context tyvars cons = do
      let params = fmap (\v -> subst (VarT (tyVarName v))) tvs'
          tvs = findTypeVars params :: Set Name
